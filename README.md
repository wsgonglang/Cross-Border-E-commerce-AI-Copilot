...
- **持久执行与停止**：Agent 提交后立即返回 runId，由独立 BullMQ Worker 执行；Job 以 runId 防重并支持瞬时失败重试，停止状态通过 MySQL 传播到 Worker 的 AbortSignal，后续工具与结论不会继续落库
...
- **分层实时策略**：统一对话通过标准 `text/event-stream` 接收持久化 Agent 快照、工具进度和终态，代理不支持 SSE 时自动退回 runId 查询；底层直接聊天仍保留 HTTP Chunked Streaming 与 RAF token 合帧
...
- **Prompt Injection 边界**：系统明确工具结果、规则文档和商品文本是不可信数据；Prompt 不能绕过工具白名单、RBAC、明确写意图或人工确认
