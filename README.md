...
| 🧪 **工程底线**    | 192 项自动化测试、核心界面双语治理、路由懒加载、Docker Compose 一键启动        |
...
> Web 页面按路由懒加载，基础框架库按组拆分 vendor chunk；2026-08-15 生产构建最大业务页面 chunk 197.57 kB（gzip 60.47 kB）。复杂页面（订单、AI 对话、商品）采用薄组合层：副作用进 hooks，UI 拆为独立组件。
...
**198 项单元/组件/集成测试 + 3 条核心浏览器 E2E**覆盖：
