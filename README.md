...
面向跨境电商运营人员的 AI 全栈业务应用。阶段 0 至阶段 20 已完成数据库认证/RBAC、商家商品 SKU、多店铺与商品刊登、订单运营工作台、可复算的 Agent 工作台、具备生产力操作的 AI 会话、单商品 AI 优化闭环、受控 Agent Tool Calling、Redis/BullMQ 批量任务、带权限和引用的规则 RAG、CSV/XLSX 结构化导入、AI 成果中心、Web 测试与路由级性能基础、订单页与 AI 对话页模块化、TanStack Query 服务端状态治理，以及 Request ID、CI、Docker 和交付文档。
...
AI 业务 Agent 的每次运行、回答、用量和工具轨迹都会保存为 `AgentRun/AgentToolCall`。运营人员可从侧栏“AI 成果中心”按类型和状态查询 Agent 运行、商品优化草稿和结构化导入任务，直接进入准确商品草稿、原批量任务或导入详情；工作台同时展示待人工确认草稿数量和最近成果。成果中心只做统一索引，商品草稿的最终状态仍以 `ProductOptimization` 为准。
...
订单运营页采用薄页面组合层：查询/分页、保存视图、详情刷新和批量操作由独立 hooks 管理，筛选、表格、批量结果、详情 Drawer 与时间线由同模块组件承载。拆分不改变服务端 RBAC、订单状态机、商家/店铺隔离或幂等边界。

AI 对话页同样采用薄页面组合层：会话导航、流式消息、停止生成、收藏/业务关联和内部分享分别由独立 hooks 与组件承载。流式接口、服务端会话持久化、归档删除前置条件和分享授权边界保持不变。
...
- “结构化导入”支持 CSV/XLSX、工作表与表头选择、字段映射和行级预览；预览阶段不写业务数据，文件限制为 5 MB、10 个工作表、1000 行和 80 列，不执行公式、宏或外部链接；
- 导入确认后由独立 BullMQ Worker 逐行调用 Product/SKU Service，支持失败重试、文件哈希、幂等键和取消未执行项；MySQL 保存最终任务、行状态、失败原因和审计记录；
- 无效行不会阻塞有效行，失败明细可下载为 CSV；可选择仅创建商品草稿，或在导入后创建指定语言的 AI 优化草稿，正式商品仍须人工确认后写回；
- 可使用 [商品与 SKU 导入模板](docs/examples/product-import-template.csv) 快速演示。商品导入不接受 Markdown、TXT 或 PDF。
...
1. 进入“结构化导入”，选择 [示例 CSV](docs/examples/product-import-template.csv)，设置表头行并分析；
2. 选择工作表并完成商品、SKU、价格、币种和库存字段映射；
3. 先查看有效、无效、警告及覆盖风险，确认此时商品数据尚未写入；
4. 选择“仅导入商品草稿”或“导入后创建 AI 优化任务”，提交后观察 Worker 进度；
5. 打开任务详情，查看逐行商品、SKU、失败原因和结果链接；有失败时下载失败明细 CSV；
6. 在“AI 成果中心”打开同一导入任务；AI 模式还会出现待人工审核的商品优化草稿。

订单运营工作台演示步骤：

1. 进入“订单”，组合选择店铺、生命周期、支付、履约、时间和金额筛选，并调整排序和可见列；
2. 保存当前视图，重置页面后重新选择该视图，确认筛选、排序、列和分页配置恢复；
3. 同时选择状态不同的订单执行批量确认或发货，检查弹窗逐单展示成功与状态机拒绝，而不是把整批显示为成功；
4. 打开订单详情，检查商品、客户、地址、金额、物流、状态事件和操作者时间线；
5. 切换到“订单 Agent”，查询当前订单状态，确认工具轨迹只有订单运营字段，不包含客户邮箱或完整地址；
6. 从工作台点击“待处理订单”，确认订单页自动应用 `PENDING/CONFIRMED/REFUNDING`，与待办数字使用同一口径。
