...
面向跨境电商运营人员的 AI 全栈业务应用。当前已完成融合工程骨架、数据库认证/RBAC、商家商品 SKU 管理、订单与经营看板、AI 会话底座、单商品 AI 优化闭环，以及受控的 Agent Tool Calling；下一阶段将增加批量 AI 任务。
...
当前自动化测试覆盖认证、角色守卫、商家隔离、商品分页隔离、负库存、订单状态机、看板聚合、AI 会话隔离、流式消息持久化、停止生成、三种目标语言结构化校验、Provider 失败落库、人工确认写回、重复确认幂等、商品版本冲突、Agent 工具规划、参数拒绝、工具审计、草稿授权边界和规则检索不足。生产构建仍有前端主包体积提示，AI 会话列表仍有 Ant Design `List` 弃用提示，后续阶段将结合路由懒加载和组件升级处理。
