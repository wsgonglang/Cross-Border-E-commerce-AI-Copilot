...
- **双模式对话**：普通对话 / 业务 Agent。Agent 模式只调用服务端白名单工具
- ** Tool Calling**：商品/SKU/库存查询、订单状态、经营概览、规则检索、创建优化草稿
- **流式回复**：SSE 流 + `AbortController` 停止生成，部分回复自动持久化
...
1. 进入 **AI 运营助手** → 切换到 **业务 Agent**
2. 依次尝试：商品库存查询 → 演示订单查询 → 经营看板 → 规则检索
3. 查看回答下方的工具名、状态、参数和裁剪后的结果
4. 输入"为 P-DEMO-001 创建西班牙语优化草稿" → 返回草稿 ID
5. 回到商品管理中进行人工确认 — 正式商品不会被 Agent 直接修改

### AI 会话生产力演示

1. 新建会话 → 重命名、分组、置顶 → 用标题或消息搜索
2. 收藏消息 → 使用 `P-DEMO-001` 或演示订单号关联业务对象
3. 导出脱敏 Markdown/JSON（无客户邮箱/电话/地址）
4. 分享给同商家只读用户 → 撤销分享 → 原链接显示 410
5. 归档会话 → 确认从活动列表消失 → 二次确认永久删除
...
> Web 页面按路由懒加载，构建最大 JS chunk 375 kB。复杂页面（订单、AI 对话）采用薄组合层：副作用进 hooks，UI 拆为独立组件。
...
- AI 会话（流式持久化、停止生成、归档/删除、收藏/关联、导出脱敏、分享授权/撤销/过期）
- 商品优化（三种语言结构化校验、人工确认写回、版本冲突、重复确认幂等）
- Agent Tool Calling（参数校验、工具审计、订单脱敏、viewer 只读边界）
...
> **作品集说明**：本项目是校招"AI 全栈偏前端"岗位作品集。阶段 0-22 已完成全部业务闭环与工程治理，阶段 23（当前）聚焦 README 首屏、截图、短演示和架构图，不再增加业务领域。不暴露真实密钥或个人数据。
