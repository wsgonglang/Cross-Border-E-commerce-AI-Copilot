...
面向跨境电商运营人员的 AI 全栈业务应用。阶段 0 至阶段 22 已完成数据库认证/RBAC、商家商品 SKU、多店铺与商品刊登、订单运营工作台、可复算的 Agent 工作台、具备生产力操作的 AI 会话、单商品 AI 优化闭环、受控 Agent Tool Calling、Redis/BullMQ 批量任务、带权限和引用的规则 RAG、CSV/XLSX 结构化导入、AI 成果中心、Web 测试与路由级性能基础、订单页与 AI 对话页模块化、TanStack Query 服务端状态治理、核心链路中英文界面与金额日期本地化、可追溯 AI 质量度量，以及 Request ID、CI、Docker 和交付文档。
...
侧栏“AI 质量度量”按当前商家提供 7/30/90 天统计：草稿接受率来自人工应用/拒绝状态，工具成功率来自持久化工具轨迹，Agent 失败率只统计终态运行，平均时延来自运行创建/完成时间，Token 来自已保存模型用量。页面展示分子、分母、每日记录和可打开的原始轨迹；无样本时显示无证据，不把固定评估集或空数据包装成生产质量结论。
...
登录页右上角“界面语言”可切换中文/English，登录后相同入口位于顶部商家/店铺业务上下文栏。界面语言会持久化，且不会改变 AI 商品优化所选的目标语言。登录页的演示账号区可一键填充 admin、operator 或 viewer 账号。
...
当前 125 项自动化测试覆盖认证、前端认证/角色路由、登录页语言入口与演示角色填充、界面语言持久化与金额日期格式化、AI 质量聚合/空样本/商家及时间窗口缓存隔离、服务端状态缓存键与精确失效、订单筛选与批量操作组件、AI 对话发送/停止/收藏/业务关联组件、服务端角色守卫、商家隔离、店铺与刊登隔离、店铺币种约束、订单/看板/Agent 店铺上下文、运营指标与上一周期复算、viewer 只读 Agent 边界、商品分页隔离、负库存、订单多维状态机、组合筛选、保存视图恢复、批量部分失败与幂等、订单事件追溯、Agent 订单投影脱敏、AI 会话隔离、流式消息持久化、停止生成、人工标题保护、归档与永久删除前置条件、消息收藏和业务关联、导出与分享脱敏、分享授权/过期/撤销、三种目标语言结构化校验、Provider 失败落库、人工确认写回、重复确认幂等、商品版本冲突、Agent 工具规划、参数拒绝、工具审计、AgentRun 成功/失败持久化、成果聚合与状态过滤、草稿授权边界、批量任务的商家隔离、幂等、取消、Worker 重试、最终失败与重复投递、规则 RAG 基础评估、CSV/XLSX 解析边界、导入预览零写入、行级校验、映射键序无关幂等、导入 Worker 成功/失败，以及 Request ID 和日志注入防护。Web 页面和 AppLayout 已按路由懒加载；新增质量页保持路由级独立分包，生产构建仍未出现 500 kB 大包提示。
