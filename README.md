...
- **流式回复**：HTTP 流 + 会话级 RAF 缓冲合帧 + `AbortController` 定向停止；结束或停止时强制刷新尾部内容，部分回复自动持久化
- **跨会话后台生成**：消息、生成状态和取消控制按会话隔离；切换后原会话继续生成，新会话可立即使用，后台结果不会覆盖当前视图
- **分层实时策略**：普通对话按动画帧合并 token 渲染；Agent 工具轨迹按 1.2 秒轮询持久化运行记录，终态自动停止
...
- AI 会话（RAF 流式合帧、完成/停止强制刷新、流式持久化、会话级后台生成与定向取消、跨会话竞态隔离、活动分支上下文隔离与窗口截断、归档/删除、收藏/关联、导出脱敏、分享授权/撤销/过期）
- 商品优化（三种语言结构化校验、人工确认写回、版本冲突、重复确认幂等）
- Agent Tool Calling（受控 ReAct 循环、依赖链两轮规划、预算上限、参数校验、工具审计、订单脱敏、viewer 只读边界、规划评估集工具选择准确率 1.0）
...
> **作品集说明**：本项目是校招“AI 全栈偏前端”岗位作品集。阶段 0-25 已完成业务闭环、工程治理、作品集呈现、跨会话流式竞态治理和 RAF 渲染合帧；后续只修复可验证的体验与质量缺口，不再增加业务领域。不暴露真实密钥或个人数据。
