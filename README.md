...
- **离线评估**：32 条固定用例覆盖直接命中、改写、中英混合与无答案场景，CI 守住 Hit@1 ≥ 0.85、Recall@3 ≥ 0.95、MRR ≥ 0.90、拒答准确率 ≥ 0.875；不将小型固定集冒充生产效果

在仓库根目录运行 `npm run rag:eval` 可查看四项指标的具体数值、未排在 Top 1 的问题和错误拒答明细；任何指标低于门槛时命令返回非零退出码。
