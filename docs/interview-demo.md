...
- 指出“模型风险提示”不等于规则 RAG 结论；需要平台合规判断时必须进入带原文引用的规则检索链路。
- 强调此时正式商品版本未变化。
- 人工确认后写回，展示版本增长和审计。
...
- 演示默认不开启草稿授权时，即使输入包含“优化”也不会向模型暴露写工具；随后由 operator 主动开启“允许本次创建草稿”，说明授权随 AgentRun 持久化并由 Worker 从 MySQL 恢复。
...
- 点击停止并说明：前端关闭订阅后仍向服务端提交取消；若停止早于 runId 返回会自动补发，服务端用条件更新让“取消”和“完成”只保留一个终态，最终消息与完成状态同事务落库。
- 对完成回答提交“有帮助/没帮助”；点踩时选择工具错误、数据不准、回答不完整或引用问题，在 AI 质量页展示原因分布。若本地显式配置真实模型凭证，可运行 `npm run agent:eval:real`；强调它是小型离线比较集，不进入普通 CI。
